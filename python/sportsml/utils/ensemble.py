...
def ensemble_predict(graph, models=None, model_dir=None, model_class=GraphModel):
    ...
    pred_graph.edata['home_pred'] = torch.stack([p.edata['home_pred'] for p in preds]).mean(axis=0)
    pred_graph.edata['away_pred'] = torch.stack([p.edata['away_pred'] for p in preds]).mean(axis=0)
    
    pred_graph.edata['neutral_pred'] = (pred_graph.edata['home_pred'] + pred_graph.edata['away_pred']) / 2
