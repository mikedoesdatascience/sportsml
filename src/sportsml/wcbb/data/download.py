...
def download(output_file: str = None, year: int = 2026):
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        api.competition_download_file(
            ...
            file_name="WRegularSeasonDetailedResults.csv",
            path=temp_dir,
        )
        api.competition_download_file(
            f"march-machine-learning-mania-{year}",
            file_name="WNCAATourneyDetailedResults.csv",
            path=temp_dir,
        )
        games = pd.concat(
            [
                pd.read_csv(f"{temp_dir}/WRegularSeasonDetailedResults.csv"),
                pd.read_csv(f"{temp_dir}/WNCAATourneyDetailedResults.csv"),
            ]
        )
...
def format_games(games, team_id_offset=3101):
